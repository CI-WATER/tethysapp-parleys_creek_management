...
def runLittleDellGoldSim(arguments,resultsFile):
    ...
    """
    #''' Comment out this line to toggle
    wps = WebProcessingService('http://ci-water.byu.edu:9999/wps/WebProcessingService', verbose=False, skip_caps=True)
    '''
    wps = WebProcessingService('http://ciw-52north.cloudapp.net:8080/52n-wps/WebProcessingService', verbose=False, skip_caps=True)
    #'''
    
    # wps.getcapabilities()
            
    processid = 'goldsim.littledell'
...
#     execution.getOutput(resultsFile)
